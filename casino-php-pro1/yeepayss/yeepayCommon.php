...
{  
	$r0_Cmd		= trim($_REQUEST['r0_Cmd']);
	$r1_Code	= trim($_REQUEST['r1_Code']);
	$r2_TrxId	= trim($_REQUEST['r2_TrxId']);
	$r3_Amt		= trim($_REQUEST['r3_Amt']);
	$r4_Cur		= trim($_REQUEST['r4_Cur']);    // 获取订单货币单位
	$r5_Pid		= trim($_REQUEST['r5_Pid']);    // 获取产品ID
	$r6_Order	= trim($_REQUEST['r6_Order']);  // 获取订单ID
	$r7_Uid		= trim($_REQUEST['r7_Uid']);    // 获取产品ID
	$r8_MP		= trim($_REQUEST['r8_MP']);     // 获取商户私有参数
	$r9_BType	= trim($_REQUEST['r9_BType']);  // 获取订单ID
	$hmac		= trim($_REQUEST['hmac']);      // 获取加密字符串
