	...
	<table class="tablesorter" cellspacing="0">
		...
		<?php if($data['data']) foreach($data['data'] as $var){ ?>
			<tr data-code='<?=json_encode($var)?>'>
				...
				<td><?php if($var['lotteryNo'] || $var['isDelete']==1 || $var['kjTime']<$this->time){ ?>--<?php }else{ ?><a href="/admin778899.php/business/betInfoUpdate/<?=$var['id']?>" button="修改:dataAddCode|取消:defaultCloseModal" title="修改投注信息" width="510" target="modal" modal="true">修改</a></td><?php } ?>
			</tr>
		<?php }else{ ?>
    <tr>
