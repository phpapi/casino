...
function gamePostCode(){
	...
	$('#wanjinDialog').html(tipString).dialog({
		...
		"确定购买": function() {
			...
			/*var rsa_n = "";
			setMaxDigits(131);
			var key= new RSAKeyPair("10001",'', rsa_n);

			code[0]['fanDian']=base64_encode(encryptedString(key,String(code[0]['fanDian'])));
			code[0]['bonusProp']=base64_encode(encryptedString(key,String(code[0]['bonusProp'])));
			code[0]['mode']=base64_encode(encryptedString(key,String(code[0]['mode'])));
			code[0]['beiShu']=base64_encode(encryptedString(key,String(code[0]['beiShu'])));
			code[0]['orderId']=base64_encode(encryptedString(key,String(code[0]['orderId'])));
			code[0]['actionNum']=base64_encode(encryptedString(key,String(code[0]['actionNum'])));
			code[0]['weiShu']=base64_encode(encryptedString(key,String(code[0]['weiShu'])));
			code[0]['playedGroup']=base64_encode(encryptedString(key,String(code[0]['playedGroup'])));
			code[0]['playedId']=base64_encode(encryptedString(key,String(code[0]['playedId'])));
			code[0]['type']=base64_encode(encryptedString(key,String(code[0]['type'])));
			data['type']=base64_encode(encryptedString(key,String(game.type)));
			data['actionNo']=base64_encode(encryptedString(key,String(actionNo.actionNo)));
			data['kjTime']=base64_encode(encryptedString(key,String(actionNo.actionTime)));*/
